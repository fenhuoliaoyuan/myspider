...
def download_1(ts_url):
    name_ts = ts_url.split('/')[-1].split('?')[0]
    if not os.path.exists('C:\pmv_ts存放区\\' + name_ts):
        ...
        if ts is not None:
            ...
            with open('C:\pmv_ts存放区\\' + name_ts, 'wb') as wt:
                wt.write(ts_open)
                # print(name_ts)
                tq.update(1)
...
if __name__ == '__main__':
    ...
    while True:
        while True:
            path_name = r'D:\hhh\jav_pmv\\'+input('输入影片名(输入完成请直接按回车)：') + '.ts'
        ...
        for path_and_m3u8 in path_and_m3u8_list:
            ...
            print(url_m3u8)
            # path_name = r'D:\hhh\jav_pmv\MIMK-082 超想上這身體！！ 就是這樣、嘗試拜託全裸母親。 篠田優.ts'
            if not os.path.exists(path_name) and not os.path.exists(path_name.replace('.ts','.mp4')):
                ...
                if len(list(get_cryptor(url_m3u8))) == 2:
                    ...
                    while len(list_ts_file) < len(ts_list):
                        with ThreadPoolExecutor(10) as tp:
                            ...
                            list_ts_file = os.listdir('C:\pmv_ts存放区')
                    tq.close()
                    print('ts下载完成')
                else:
                ...
                with open(path_name, 'ab') as ab:
                    for i in [j.split('/')[-1].split('?')[0] for j in ts_list]:
                        with open('C:\pmv_ts存放区\\' + i, 'rb') as rb:
                            ab.write(rb.read())
                    print(path_name+'合并完成')
                for j in ['C:\pmv_ts存放区\\' + i for i in list_ts_file]:
                    # print(j)
                    os.remove(j)
                print('ts删除完成')
            # fanhao_zhangma("E:\番号")
        fanhao_zhangma(r"D:\hhh\jav_pmv")
