...
def links(url):
    ...
    link_list = data.xpath('//*[@class="listchannel"]/a/@href')
    return link_list
...
def relink(link):
    res = requests.get(link, headers=header, timeout=20).text
    data = etree.HTML(res)
    cname = data.xpath('//*[@id="viewvideo-title"]/text()')[0]
...
def load(url, name):
    ...
    if os.path.exists('F:/绅士/91/%s.mp4' % (name)) == True:
        print('文件已存在')
        pass
    else:
        file_size = int(res.headers['content-length'])
        pdar = tqdm(ncols=70,total=file_size, desc=name, unit='it', unit_scale=True)
        with open(r'F:/绅士/91/%s.mp4' % (name), 'wb')as f:
