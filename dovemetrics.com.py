...
class Dovemetrics(object):
    ...
    def parseText(cls, response):
        ...
        if response is not None:
            res = response.json()
            choices_TICCKER = res['data']['tableSchemas'][4]['columns'][2]['typeOptions'][
                'choices']  # 'data.tableSchemas[4].columns[2].typeOptions.choices'
            choices_LOCATION = res['data']['tableSchemas'][4]['columns'][10]['typeOptions'][
                'choices']  # ‘data.tableSchemas[4].columns[10].typeOptions.choices
            ...
            for row in rows:
                ...
                try:
                    ...
                    YEAR_FOUNDED = None
                try:
                    FOUNDER = row['cellValuesByColumnId']['fldbGZmPcgWw1UomW']['valuesByForeignRowId'][
                        'recNniOlJpoflM0L4']
                    ...
                    FOUNDER = None
                try:
                    WEBSITE = row['cellValuesByColumnId']['fldiL1KEOZO4MHcvW']['valuesByForeignRowId'][
                        'recNniOlJpoflM0L4']
...
if __name__ == '__main__':
    ...
    res = Dovemetrics.get_response(url=url)
    data = Dovemetrics.parseText(response=res)
    print(data)
    Dovemetrics.saveToFile(data=data)
