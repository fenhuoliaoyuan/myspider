...
def get_href_list(start_page, end_page, URL):
    ...
    for page in range(int(start_page), int(end_page) + 1):
        ...
        URL_0 = 'https://mdr18.pw/index.php/vod/search/page/'
...
def download_1(ts_url):
    ...
    if not os.path.exists('.\测试\\' + name_ts):
...
def download_2(jpg_url):
    ...
    if not os.path.exists('.\测试\\' + name_jpg):
        ...
        if ts is not None:
            ...
            # ts_open = cryptor.decrypt(ts)
            # dict_ts[name_ts]= ts_open
            with open('.\测试\\' + name_jpg, 'wb') as wt:
                wt.write(ts)
                print(name_jpg)
...
if __name__ == '__main__':
    ...
    path_root = r'F:\国产av\苡若'
    if not os.path.exists(path_root):
        os.mkdir(path_root)
    start_time = int(time.time())
    # 使用搜索所得页地址，获取href_list
    url = 'https://mdr18.pw/index.php/vod/search/page/1/wd/TM0086.%E8%80%81%E5%85%AC%E5%81%B7%E7%AA%A5%E6%88%91%E4%B8%8E%E5%81%A5%E8%BA%AB%E6%95%99%E7%BB%83%E7%9A%84%E5%81%B7%E6%83%85%E8%AE%AD%E7%BB%83.html'
    ...
    for url_detail in url_detail_list:
        ...
        if not os.path.exists(path_mp4):
            ...
            with open(path_mp4, 'ab') as ab:
                for i in ts_list:
                    with open('测试\\' + i.split('/')[-1], 'rb') as rb:
                        ab.write(rb.read())
                print(path_mp4 + '合并完成')
