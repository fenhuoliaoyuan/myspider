...
def get_href_list(start_page, end_page, URL):
    ...
    for page in range(int(start_page), int(end_page) + 1):
        ...
        if page_text:
            ...
            href_list = ['https://www.porn5f.com' + href for href in tree.xpath('//div[@class="well well-sm"]/a/@href')]
...
if __name__ == '__main__':
    ...
    path_root = r'F:\国产av\糖心Vlog'
    if not os.path.exists(path_root):
        os.mkdir(path_root)
    start_time = int(time.time())
    # 使用搜索所得页地址，获取href_list
    url = 'https://www.porn5f.com/search/videos?search_query=%E7%B3%96%E5%BF%83'
    url_detail_list = get_href_list(1, 2, url)
    with ThreadPoolExecutor(8) as ttp:
        for url_detail in url_detail_list:
