...
def get_cryptor(url_m3u8):
    ...
    for line in m3u8_txt.split('\n'):
        # print(line)
        if 'URI' in line:
            url_key = '/'.join(url_m3u8.split('/')[0:-1]).replace('i.baobuzz.com','dweb.link')+'/' + re.compile('URI="(.*)"').findall(line)[0]
    ...
    for page in range(1,4):
        ...
        if page_text_ is not None:
            ...
            for row in article_list:
                ...
                title = '激情丛林 Tarzan-X_ Shame of Jane (1995)'
                ...
                if not os.path.exists(path_name) and not os.path.exists(path_name.replace('.ts','.mp4')):
                    ...
                    if page_text is not None:
                        ...
                        # token = re.compile('var token = \"(.*?)\";').findall(page_text)[0]
                            ...
                            url_m3u8 = 'https://i.baobuzz.com/ipfs/QmTMqopFAH28tFMmQ52W7Um3GSeMqxWDoLx3vAiezb1jKC/dan.m3u8'
                            ...
                            if len(list(tuple_test)) == 2:
                                ...
                                ts_list = ['/'.join(url_m3u8.split('/')[:-1]).replace('i.baobuzz.com','dweb.link') + '/' + i for i in
                                           ts_list]
                                tq = tqdm(total=len(ts_list))
